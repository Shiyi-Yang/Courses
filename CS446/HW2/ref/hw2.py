...
def svm_solver(x_train, y_train, lr, num_iters,
    ...
    def K(x_train): # form matrix with elements K_ij to be kernel(x_i, x_j)
        results = torch.zeros([n,n])
        
        for i in range(n):
            for j in range(n):
                results[i,j] = kernel(x_train[i],x_train[j])
                
        return results
    
    def f(alpha): # form function f with argument alpha, return a scalar    
        A = torch.outer(alpha,alpha) 
        Y = torch.outer(y_train,y_train) 
        AY = A*Y # matrix with elements AY_ij to be alpha_i * alpha_j * y_i * y_j
        
        P1 = AY*Ker # each element in matrix is (alpha_y)_i * (alpha_y)_j * kernel(x_i,x_j)
        P1s = 1./2 * P1.sum()
...
def svm_predictor(alpha, x_train, y_train, x_test,
    ...
    for j in range(m):
        for i in range(n):
            y_test[j] += alpha[i] * y_train[i] * kernel(x_test[j],x_train[i])
